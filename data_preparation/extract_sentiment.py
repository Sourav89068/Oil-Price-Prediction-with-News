...
class Sentiment_Classifier:
    ...
    def sentiment_scores(self,text: str)-> List:
        inputs=tokenizer(text,return_tensors="pt",padding=True)
        outputs=finbert(**inputs)[0]
        sentiment_array=outputs.detach().numpy()[0]
        return sentiment_array
    
    def sentiments_from_scores(self,score_array:List)-> str:
        labels = {0:'neutral', 1:'positive',2:'negative'}
        label=labels[np.argmax(score_array)]
        return label

    def sentiments_with_score_and_labels(self) -> pd.core.frame.DataFrame:
        data=self.data.copy()
